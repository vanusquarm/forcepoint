...
export default function Table<T extends Record<string, string | number>>({
  ...
  const handleExportAll = () => {
    ...
    const csvContent = [
      columns
        .map((col) => (col as ColumnDefResolved<T>).accessorKey)
        .join(","), // Header
      ...allData.map((row) =>
        columns
          .map((col) => row[(col as ColumnDefResolved<T>).accessorKey ?? ""])
  ...
  return (
    <ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
      <li
        ...
        onClick={() => {
          table.previousPage();
          setActiveNavBtn((curr) => (!table.getCanPreviousPage() ? (curr -= 1) : 1));
        }}
      >
        <a
          href="#"
      ...
      <li
        ...
        onClick={() => {
          ...
          setActiveNavBtn((curr) => (!table.getCanNextPage() ? (curr += 1) : table.getPageCount()));
        }}
      >
        <a
          href="#"
